...
def generate_forecast(fh, df, df_features, forecast_horizon, model_config, time_series_column_group, target_columns, feature_columns):
    ...
    tide_model = dm.TFTModel(**model_config, output_chunk_length = forecast_horizon)
    suppress_output(tide_model.fit, TimeSeries.from_dataframe(y_train, 'date'), future_covariates = TimeSeries.from_dataframe(pd.concat([x_train[['date', 'time_sine']], x_test[['date', 'time_sine']]]), 'date'))
    predictions = suppress_output(tide_model.predict, forecast_horizon, future_covariates = TimeSeries.from_dataframe(pd.concat([x_train[['date', 'time_sine']], x_test[['date', 'time_sine']]]), 'date')).values()
...
def generate_tft_darts_stacked_residuals(
